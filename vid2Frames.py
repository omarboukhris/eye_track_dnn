...
def grayscale_and_resize(videoFiles, targetFiles, downSample, outputPath):
    ...
    vid_nums = []
    frame_nums = []
    for file_path in tqdm.tqdm(targetFiles):
    ...
    for file_path in tqdm.tqdm(videoFiles):
...
train_output_folder = str(Path(__file__).parent / 'LPW/train_frames/') + "/"
test_output_folder = str(Path(__file__).parent / 'LPW/test_frames/') + "/"
valid_output_folder = str(Path(__file__).parent / 'LPW/valid_frames/') + "/"
